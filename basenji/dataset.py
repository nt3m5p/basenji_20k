...
class SeqDataset:
    def __init__(self, data_dir, split_label, batch_size, shuffle_buffer=128,
                 seq_length_crop=None, mode='eval', tfr_pattern=None, tissues_scale=False, targets_scale=False,
                 weight_type=None):
        ...
        self.tissues_scale = tissues_scale
        ...
        self.weight_contain = "weight" in self.data_dir
        self.make_dataset()
    ...
    def generate_parser(self, raw=False, w_low=0.5, w_high=2):

        def tau_value(vec):
            """vec: [num_tissues] 向量 → 标量 τ（0–1）"""
            vmax = tf.reduce_max(vec)
            # 如果全 0，直接返回 0
            zero_case = lambda: tf.constant(0., dtype=tf.float32)
            compute_case = lambda: (
                    tf.reduce_sum(1. - vec / vmax) /
                    (tf.cast(tf.size(vec), tf.float32) - 1.)
            )
            return tf.cond(tf.equal(vmax, 0.), zero_case, compute_case)

        def entropy_specificity(vec, eps=1e-8):
            """
            输入：vec - shape [num_tissues]
            输出：范围 [0,1] 的组织特异性分数（越高越特异）
            """
            vec = tf.cast(vec, tf.float32)
            vec_sum = tf.reduce_sum(vec) + eps
            p = vec / vec_sum  # 概率分布
            entropy = -tf.reduce_sum(p * tf.math.log(p + eps))
            h_max = tf.math.log(tf.cast(tf.size(vec), tf.float32))
            return 1.0 - entropy / h_max  # specificity ∈ [0, 1]

        def align_weights(w, L, C, dtype=tf.float32):
            return tf.broadcast_to(w, [L,C])

        def parse_proto(example_protos):
            """Parse TFRecord protobuf."""
            # define features
            if self.weight_contain:
                features = {
                    TFR_INPUT: tf.io.FixedLenFeature([], tf.string),
                    TFR_OUTPUT: tf.io.FixedLenFeature([], tf.string),
                    TFR_WEIGHT: tf.io.FixedLenFeature([], tf.string)
                }
            else:
                features = {
                    TFR_INPUT: tf.io.FixedLenFeature([], tf.string),
            ...
            if self.weight_contain:
                # ---------- decode weight ----------
                if self.weight_type == "bin":  # 每个 bin 一个权重
                    weight = tf.io.decode_raw(parsed_features['weight'], tf.float16)
                    weight = tf.reshape(weight, [self.target_length])  # [L]
                    weight = tf.cast(weight, tf.float32)
                    weight = tf.expand_dims(weight, -1)  # [L, 1]   ✅最后一维=1
                elif self.weight_type == "seq":  # 整条序列一个权重
                    weight = tf.io.decode_raw(parsed_features['weight'], tf.float16)
                    weight = tf.reshape(weight, [])  # 标量
                    weight = tf.cast(weight, tf.float32)
                else:  # 默认全 1
                    weight = tf.constant(1., tf.float32)  # 标量
            else:
                weight = tf.ones([1], dtype=tf.float32)  # 默认给每个目标一个权重

            # if (targets.shape[-1]) == 80:
            #     tf.print(sequence.shape, targets.shape, weight.shape)
            #     tf.print(weight)  # 打印权重

            if self.tissues_scale:
                # 1. 在 bin 维度求平均 → [num_targets]
                tgt_mean = tf.reduce_mean(targets, axis=0)
                # 2. 计算 τ
                tau = tau_value(tgt_mean)  # 标量
                weight = 0.6 + (1.8 - 0.6) * tau  # 标量
                # tf.print("tau weight =", weight)
                # 2. 计算 entropy_specificity
                spec = entropy_specificity(tgt_mean)  # 0–1
                spec = tf.sqrt(spec)  # 把 0.01 放大成 0.1
                weight = 0.6 + (6 - 0.5) * spec
            if self.targets_scale:
                weight = weight * (self.num_targets / 48) ** 0.5


            return sequence, targets, weight
    ...
    def compute_stats(self):
        ...
        if self.num_seqs > 0:
            ...
            print('%s has %d sequences with %d/%d targets' % (self.tfr_path, self.num_seqs, self.num_targets_nonzero,
                                                              self.num_targets), flush=True)
        else:
    ...
    def numpy(self, return_inputs=True, return_outputs=True, step=1, target_slice=None, dtype='float16'):
        ...
        for seq_raw, targets_raw, _ in dataset:
            # sequence
            if return_inputs:
                seq_1hot = seq_raw.numpy().reshape((self.seq_length, -1))
                if self.seq_length_crop is not None:
...
class RnaDataset:
    ...
    def make_parser(self):  # , rna_mode
        def parse_proto(example_protos):
            ...
            weight = tf.io.decode_raw(feature_tensors['weight'], tf.float16)
            weight = tf.cast(weight, tf.float32)
