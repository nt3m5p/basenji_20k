...
def snp_seq1(snp, seq_len, genome_open):
  ...
  if seq_start <= 0:
    ...
    #seq = 'N'*(1-seq_start) + genome_open.fetch(snp.chr.replace("chr","Chr"), 0, seq_end).upper() # pysam 读到的fasta文件为Chr 所以这里要replace
  else:
    seq = genome_open.fetch(snp.chr, seq_start - 1, seq_end).upper()
    #seq = genome_open.fetch(snp.chr.replace("chr","Chr"), seq_start - 1, seq_end).upper() # pysam 读到的fasta文件为Chr 所以这里要replace
