...
def poisson_multinomial(y_true, y_pred, total_weight=1, epsilon=1e-6, rescale=False, sample_weight=None):
    # y_true.shape [B,L,T] batch size, targets length, targets
    seq_len = y_true.shape[1]  # 需要确保该维度一致

    # add epsilon to protect against tiny values
    y_true += epsilon
    y_pred += epsilon

    # sum across lengths for Poisson loss
    s_true = tf.reduce_sum(y_true, axis=-2, keepdims=True)
    s_pred = tf.reduce_sum(y_pred, axis=-2, keepdims=True)

    # normalize to sum to one
    p_pred = y_pred / s_pred

    # Total count Poisson loss (B x T)
    poisson_term = tf.keras.losses.poisson(s_true, s_pred)  # B x T
    poisson_term = tf.reduce_mean(poisson_term, axis=-1) # B

    # Multinomial loss (B x L x T)
    pl_pred = tf.math.log(p_pred)
    multinomial_dot = -tf.math.multiply(y_true, pl_pred)  # B x L x T
    multinomial_term = tf.reduce_mean(multinomial_dot, axis=-1)  # B x L

    # sample_weight shape [B, L, 1]
    tf.print(sample_weight)
    if sample_weight is not None:
        # sample_weight 允许 [B], [B,L], [B,L,1]
        sw = tf.convert_to_tensor(sample_weight, tf.float32)
        if tf.rank(sw) == 1:               # [B] - 先 broadcast 到 [B,T]
            sw = tf.expand_dims(sw, -1)    # [B,1]
        elif tf.rank(sw) == 2:             # [B,1]
            pass
        elif tf.rank(sw) == 3:             # [B,L,1]
            sw = tf.reduce_mean(sw, axis=-1)
        tf.print(tf.shape(sw))

        multinomial_term = multinomial_term * sw

    multinomial_term = tf.reduce_mean(multinomial_term, axis=-1)  # B

    poisson_loss_sum = tf.reduce_sum(poisson_term)  # 求和 Poisson 损失
    multinomial_loss_sum = tf.reduce_sum(multinomial_term)  # 求和 Multinomial 损失

    loss = poisson_loss_sum / seq_len + 5.0 * multinomial_loss_sum


    return loss


class PoissonMultinomial(tf.keras.losses.Loss):
    """
    直接按照给定的 `multinomial_resolution` 对 Poisson 项做长度归一化，
    并保持 1×Poisson + 5×Multinomial 的权重；不再强制把序列切成 8 段。
    返回逐元素 (B, L, T) 损失，后续由 Keras 用 `sample_weight` 聚合。
    """
    def __init__(self,
                 multinomial_resolution: int = 224,
                 reduction: str = tf.keras.losses.Reduction.NONE,   # ★放到这里
                 name: str = "poisson_multinomial",
                 **kw):
        super().__init__(name=name, reduction=reduction, **kw)
        self.mr = float(multinomial_resolution)

    def call(self, y_true, y_pred):
        eps = 1e-7
        y_true = y_true + eps
        y_pred = y_pred + eps
        # -------- Poisson NLL（按 multinomial_resolution 归一化）--------
        s_true = tf.reduce_sum(y_true, axis=1, keepdims=True)      # (B,1,T)
        s_pred = tf.reduce_sum(y_pred, axis=1, keepdims=True)
        pois   = (s_pred - s_true * tf.math.log(s_pred)) / self.mr  # (B,1,T)
        pois   = tf.broadcast_to(pois, tf.shape(y_true))            # (B,L,T)

        # -------- Multinomial NLL（形状损失）--------
        p_pred = y_pred / s_pred                                     # (B,L,T)
        multi  = -y_true * tf.math.log(p_pred)                       # (B,L,T)

        # -------- 组合 --------
        return pois + 5.0 * multi        # (B,L,T)


# class PoissonMultinomial(tf.keras.losses.Loss):
#     """
#     Borzoi‑式 (Poisson + 5×Multinomial) 损失，
#     输出逐元素 (B, L, T) loss，交给 Keras 后续按 sample_weight 聚合。
#     """
#     def __init__(self, multinomial_resolution: int = 224, name="poisson_multinomial", **kw):
#         super().__init__(name=name, reduction=tf.keras.losses.Reduction.NONE, **kw)
#
#     def call(self, y_true, y_pred):
#         eps = 1e-6
#         y_true = y_true + eps
...
#
#         # 片长 L（动态取）
#         L = tf.cast(tf.shape(y_true)[1], tf.float32)
#
#         # ---- Poisson NLL（按片长度平均）----
#         s_true = tf.reduce_sum(y_true, axis=-2, keepdims=True)   # (B,1,T)
#         s_pred = tf.reduce_sum(y_pred, axis=-2, keepdims=True)
#         pois = (s_pred - s_true * tf.math.log(s_pred)) / L       # (B,1,T)
#         pois = tf.broadcast_to(pois, tf.shape(y_true))           # (B,L,T)
#
#         # ---- Multinomial NLL（形状损失）----
#         p_pred = y_pred / s_pred
#         multi = -y_true * tf.math.log(p_pred)                    # (B,L,T)
#
#         # ---- 合并：1×Poisson + 5×Multinomial ----
#         loss = pois + 5.0 * multi                                # (B,L,T)
#
#         return loss
