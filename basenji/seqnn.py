...
class SeqNN():
  ...
  def evaluate(self, seq_data, head_i=None, loss_label='poisson', loss_fn=None, np_save_path=None):
    ...
    y_pred = model.predict(seq_data.dataset)
    y_true = np.concatenate(
      [y for _, y, _ in iter(seq_data.dataset)], axis=0
    )

    np.save(os.path.join(np_save_path, '[yp,yt].npy'), [y_pred,y_true])

    # evaluate
    return model.evaluate(seq_data.dataset)#, pearson_results
