...
# Modifications:
#   - Extended SeqNN.evaluate call to support saving numpy outputs via np_save_path.
#   - Added --tissue option to aggregate targets/predictions by tissue (parsed from targets description)
#     and run downstream accuracy plots on tissue-level averages.
#   - Added optional collection of ROC curve points (fpr/tpr) for later export (currently commented).
#
...
def main():
    ...
    parser.add_option('--tissue', dest='tissue',
                      default=False, action='store_true',
                      help='Whether it is merged through organization')
    ...
    # evaluate
    test_loss, test_metric1, test_metric2 = seqnn_model.evaluate(eval_data, loss_label=loss_label, loss_fn=loss_fn, np_save_path=options.out_dir)
    ...
    if options.accuracy_indexes is not None:
        ...
        # 按照组织平均输出
        if options.tissue:
            targets_df['tissue'] = targets_df['description'].str.split('_').str[1]
            unique_tissues = targets_df['tissue'].unique()  # 保持原始出现顺序
            merged_target = []
            merged_pred = []
            for tissue in unique_tissues:
                # 获取当前组织的样本索引
                targets_df.reset_index(drop=True, inplace=True)
                indices = targets_df[targets_df['tissue'] == tissue].index.tolist()
                # 提取对应的切片并求均值（axis=2 表示在第三维求均值）
                tissue_target_mean = test_targets[:, :, indices].mean(axis=2, keepdims=True)
                tissue_pred_mean = test_preds[:, :, indices].mean(axis=2, keepdims=True)

                merged_target.append(tissue_target_mean)
                merged_pred.append(tissue_pred_mean)

            # 步骤 3: 合并所有组织的均值结果
            test_targets = np.concatenate(merged_target, axis=2)
            test_preds = np.concatenate(merged_pred, axis=2)

            accuracy_indexes = list(range(test_targets.shape[2]))
        ...
        for ti in accuracy_indexes:
            ...
            # 方案1
            scale_factor = 100
            test_targets_ti_flat = test_targets_ti_flat * scale_factor
            test_preds_ti_flat = test_preds_ti_flat * scale_factor
        ...
        # # 在循环外保存 fpr_list 和 tpr_list
        # np.save('%s/roc/fpr_list.npy' % options.out_dir, np.array(fpr_list, dtype=object))
        # np.save('%s/roc/tpr_list.npy' % options.out_dir, np.array(tpr_list, dtype=object))
