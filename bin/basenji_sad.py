...
# Modifications:
#   - Extended SAD outputs with local-window statistics (1024bp and 256bp around center bins):
#       ref_lpred/alt_lpred, lsad, lsar, ref_lpred256/alt_lpred256.
#   - Added global mean predictions per target (ref_pred/alt_pred).
#   - Added --pseudo to control log2 pseudocount used in lsar.
#   - Updated output HDF5 layout to include the above stats (per-SNP x per-target datasets).
#
...
def main():
  ...
  parser.add_option('--stats', dest='sad_stats',
      default='SAD,lsad,lsar,ref_lpred,alt_lpred,ref_pred,alt_pred,ref_lpred256,alt_lpred256',
  ...
  for si, snp in tqdm(enumerate(snps), total=len(snps)):
    ...
    combined_preds = np.stack([ref_preds, alt_preds], axis=0)  # shape: (2, ...)
    ...
    loc_mid = seqnn_model.target_lengths[0] // 2
    loc_start = loc_mid - ((1024 // 2) // 128)
    loc_end = loc_start + (1024 // 128)
    # sum locally
    ref_preds_loc = ref_preds[loc_start:loc_end, :].mean(axis=0, dtype='float32')
    alt_preds_loc = alt_preds[loc_start:loc_end, :].mean(axis=0, dtype='float32')

    # compute SAD locally
    sad_loc = alt_preds_loc - ref_preds_loc
    sar_loc = np.log2(alt_preds_loc + options.log_pseudo) - np.log2(ref_preds_loc + options.log_pseudo)

    sad_out['ref_lpred'][si] = ref_preds_loc.astype('float32')
    sad_out['alt_lpred'][si] = alt_preds_loc.astype('float32')

    sad_out['ref_pred'][si] = ref_preds.mean(axis=0, dtype='float32')
    sad_out['alt_pred'][si] = alt_preds.mean(axis=0, dtype='float32')
    sad_out['lsad'][si] = sad_loc.astype('float32')
    sad_out['lsar'][si] = sar_loc.astype('float32')

    loc_mid = seqnn_model.target_lengths[0] // 2
    loc_start256 = loc_mid - ((256 // 2) // 128)
    loc_end256 = loc_start256 + (256 // 128)
    ref_preds_loc256 = ref_preds[loc_start256:loc_end256, :].mean(axis=0, dtype='float32')
    alt_preds_loc256 = alt_preds[loc_start256:loc_end256, :].mean(axis=0, dtype='float32')
    sad_out['ref_lpred256'][si] = ref_preds_loc256.astype('float32')
    sad_out['alt_lpred256'][si] = alt_preds_loc256.astype('float32')

  # np.save(options.out_dir + "/all_combined_preds.npy", np.array(all_combined_preds))
...
def write_snp_len(ref_preds, alt_preds, sad_out, si, sad_stats):
  ...
  if 'SAD' in sad_stats:
    ...
    sad_out['SAD'][si] = sad.astype('float32')
