...
class GenomeCoverage:
  ...
  def read_bam(self, bam_file, genome_sorted=False, all_overlap=False):
    ...
    for align in pysam.AlignmentFile(bam_file):
      ...
      if not align.is_unmapped and num_maps <= self.maps_t and not align.is_duplicate:
        ...
        try:
          ...
          print(align.reference_id, chrom_pos)
          print("AssertionError 1:", e)
          print("GIS values:", gis)
    ...
    try:
      ...
      # 打印 bam_file 和错误信息
      print(f"Error processing BAM file: {bam_file}", file=sys.stderr)
      print(f"csr_matrix: {e}", file=sys.stderr)
      # 可以选择退出程序或继续处理其他部分
      sys.exit(1)  # 或者 raise，视具体需求而定
    print(' Done in %ds.' % (time.time() - t0), flush=True)
  ...
  def read_multi_bwa(self, multi_positions, multi_reads, multi_weight, align, gi, ri, align_shift_forward, align_shift_reverse):
    ...
    for multi_align_str in multi_align_strings:
      ...
      if multi_strand == '+':
        ...
        #             -30782481 -> 30782481       +    150            -  4
        if multi_pos>multi_chrom_length-1:
          print(align)
        multi_pos = min(multi_pos,multi_chrom_length-1)
  ...
  def write(self, output_file, single_or_pair, smooth_sd=0, zero_eps=.0003):
