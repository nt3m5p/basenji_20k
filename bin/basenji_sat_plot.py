...
def main():
    ...
    parser.add_option('--fig-height', dest='figure_hight',
                      default=6, type='float',
                      help='Figure hight [Default: %default]')
    ...
    for si in seq_indexes:
        ...
        for tii in tqdm(range(num_targets), desc="Plotting targets"):
            if options.targets_file is not None:
            ...
            # setup plot
            plt.figure(figsize=(options.figure_width, options.figure_hight))
            if options.gain:
                grid_rows = 4
            else:
                grid_rows = 3
            ...
            # plot heat map
            # 将 one-hot 序列转换成字符串
            ref_seq = ''.join('ACGT'[np.argmax(col)] for col in seq_1hot)

            plot_heat(ax_heat, delta_ti.T, ref_seq, options.min_limit)
...
def plot_heat(ax, sat_delta_ti, ref_seq, min_limit):
    """
    Plot satmut deltas as a heat-map and label the x-axis with the reference
    nucleotide sequence.

    Args
    ----
    ax            : matplotlib axis to plot to
    sat_delta_ti  : (4 × L) array – single-target delta matrix
    ref_seq       : str           – reference sequence of length L (ACGT…)
    min_limit     : float         – minimum heat-map limit
    """
    ...
    sns.heatmap(
        ...
        vmin=-vlim,
        vmax=vlim,
        xticklabels=False,   # 关闭 seaborn 默认 xticklabels
        ax=ax)
    ...
    # ---------- 关键：给 x 轴加上碱基序列 ----------
    # seaborn 在每格中央画格子，所以 +0.5 能让字母居中
    L = len(ref_seq)
    ax.set_xticks(np.arange(L) + 0.5)
    ax.set_xticklabels(list(ref_seq), fontsize=6)#, rotation='vertical')
